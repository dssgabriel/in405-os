...
SE_FICHIER SE_ouverture(const char *chemin, int access) 
{
	SE_FICHIER fichier;
	...
	// if (acces & O_CREAT || acces & O_TMPFILE)
		fichier.descripteur = open(chemin, access, 0644);
	else
		fichier.descripteur = open(chemin, access);
...
int SE_lectureCaractere(SE_FICHIER fichier, char *caractere) 
{
	...
	if (!(fichier.acces == O_RDONLY || fichier.acces & O_RDONLY || fichier.acces & O_RDWR)) 
	{
...
int SE_ecritureCaractere(SE_FICHIER fichier, const char caractere) 
{
	...
	{
		printf ("Erreur à l'écriture de %s : le fichier n'a pas été ouvert avec un accès en écriture\n", fichier.chemin);
		return -1;
	}
...
int SE_lectureChaine (SE_FICHIER fichier, char *chaine,	int tailleMax) 
{
	...
	do 
	{
		reti = read(fichier.descripteur, chaine + i, 1); 
		retour += reti;
		++i;
...
int SE_ecritureChaine (SE_FICHIER fichier, const char *chaine, int taille) 
{
	...
	do 
	{
		reti = write(fichier.descripteur, chaine + i, 1); 
		retour += reti;
		++i;
...
int SE_lectureEntier (SE_FICHIER fichier, int *entier) 
{
	...
	*entier = 0;
	...
	do 
	{
		reti = read(fichier.descripteur, &c, 1); 
		retour += reti;
		if (reti == -1) 
		{
			printf("Erreur à la lecture de %s : %s\n", fichier.chemin, strerror(errno));
...
int SE_ecritureEntier (SE_FICHIER fichier, const int entier) 
{
	...
	if (entier < 0) 
	{
		reti = write (fichier.descripteur, "-", 1); 
		retour += reti;
		if (reti < 0) 
		{
			printf ("Erreur à l'écriture de %s : %s\n", fichier.chemin, strerror(errno));
	...
	{
		--i;
		reti = write(fichier.descripteur, buffer + i, 1);
		retour += reti;
		if (reti < 0) 
		{
			printf ("Erreur à l'écriture de %s : %s\n", fichier.chemin, strerror(errno));
